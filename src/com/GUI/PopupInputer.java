...
public class PopupInputer extends FramelessPanel {
    ...
    public PopupInputer(int width, int height, MainWindow frameMainWindow) {
        ...
        textCityName = new BorderTextField(20, true, "请输入房间名");
        ...
        textAreaCityInfo = new RoundTextArea(20, 20, 200, 300, true, "请输入房间信息");
        ...
        labelTitle = new JLabel("请输入第n个房间", SwingConstants.CENTER);
        ...
        btnConfirm.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (textCityName.getText().equals(new String("请输入房间名"))) {
                    frameMainWindow.logToWindow("至少输入房间名称!");
                } else { //如果已经输入了房间名
                    ...
                    String cityInfo = textAreaCityInfo.textAreaReal.getText().equals(new String("请输入房间信息")) ? "" : textAreaCityInfo.textAreaReal.getText();
                    cityBtnTemp.setStrCityInfo(cityInfo);
                    cityBtnTemp.setBackground(Color.orange);
                    listTempCityBtn.add(cityBtnTemp);
                    inputCount++;
                    labelTitle.setText(String.format("请输入第%d个房间的信息", inputCount + 1));
    ...
    private void collectComplete() {
        ...
        frameMainWindow.logToWindow("录入成功,已将录入房间叠放在画板左上角,按住鼠标左键拖拽即可放置在任意位置");
        for (CityBtn temp :
                listTempCityBtn) {
        ...
        frameMainWindow.inputCity.setText("请输入录入房间个数");
        frameMainWindow.btnInputConfirm.setEnabled(true);
